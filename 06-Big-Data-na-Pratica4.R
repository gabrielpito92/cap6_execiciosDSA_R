...
library(caret) #criar modelo de ML
library(plotly) #criar gráficos
library(readxl) #ler excel
library(rfm) #para análise RFM
library(stats) #para calculo estatistico
library(factoextra) #nos permite trabalhar com algumas variaveis de forma diferente

#professor utilizou anteriormente um caminho: (Dica para utilizar diretorios dif)
...
carrega_dados <- function()
{
  ...
  dados_combinados <- rbind(sheet1, sheet2) #unindo os dados por linha/row
...
#utilizar esta cópia para treinar mudar a escala do gráfico - eixo x e y
ggplot(dataset,
       aes(x = TotalPrice)) +
  geom_density(fill = "#69b3a2", color = "#e9ecef", alpha = 3.5) +
  labs(title = 'Distribuição da Variável TotalPrice')
...
# Número de clientes
length(dataset$`Customer ID`) #usa essa aspas porque o nome da coluna tem espaço
...
calcula_rfm <- function(x){
  z <- x %>% group_by(`Customer ID`) %>% 
     summarise(Recency = as.numeric(date1 - max(InvoiceDate)), #diferença da data considerada para a data da ultima compra
               Frequency = n(), 
  ...
  Q1 <- quantile(z$Monetary, .25) #outlier comprou pouco
  Q3 <- quantile(z$Monetary, .75) #outlier comprou muito
  IQR <- IQR(z$Monetary)
...
segmenta_cliente <- function(rfm)
{
  ...
  dados_rfm <- select(rfm, c('Recency','Frequency','Monetary')) #filtrando para trazer somente as colunas que interessam
  
  # Cria o modelo
  modelo_kmeans <- kmeans(dados_rfm, center = 5, iter.max = 50) #isso aqui é machine learning
  # pegue esses dados e divida em 5 grupos de clientes para mim.
  # distancia matematica euclidiana
  # defino 50 iterações - tempo de treinamento (50 passadas)
  # quanto mais passadas, mais preciso pode ficar meu algortitmo mas, corro o risco de overfiting
...
# Executa a função
grafico <- segmenta_cliente(valores_rfm)[1] #grafico - o num 1 significa: plot
grafico

tabela_rfm <- segmenta_cliente(valores_rfm)[2] #tabela - o num 2 significa: dados
